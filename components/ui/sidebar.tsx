...
>(
  (
    ...
    const buttonChildren = (
        <>
            {React.Children.map(children, (child) => {
                if (React.isValidElement(child) && typeof child.type !== 'string' && child.type.name === 'div') {
                    return React.cloneElement(child, {
                        className: cn(child.props.className, "group-data-[collapsible=icon]/sidebar-wrapper:hidden")
                    });
                }
                if (React.isValidElement(child) && typeof child.type === 'string' && child.props.className?.includes('p-2')) {
                    // This is the icon wrapper
                    return child;
                }
                 if (typeof child === 'string') {
                    return <span className="group-data-[collapsible=icon]/sidebar-wrapper:hidden">{child}</span>;
                }
                return child;
            })}
        </>
     ...
     const buttonWithCorrectChildren = (
      <Comp
        ref={ref}
      ...
      >
        {React.Children.map(children, (child) => {
          if (React.isValidElement(child) && (child.props.className as string)?.includes('p-2')) {
            return child; // The icon
          }
          if (typeof child === 'string') {
             return <span className="group-data-[collapsible=icon]/sidebar-wrapper:hidden">{child}</span>
          }
    ...
    if (!tooltip && state === 'collapsed') {
      ...
      tooltip = textChild ? String(textChild) : undefined;
    }
    ...
    return (
      <Tooltip>
        <TooltipTrigger asChild>{buttonWithCorrectChildren}</TooltipTrigger>
        <TooltipContent
          side="right"
          align="center"
